...
function DetailRow({ icon: Icon, label, value }: { icon: typeof Tag; label: string; value: string }) {
  return (
    <div className="flex items-start gap-3 rounded-xl bg-slate-50 p-3 dark:bg-slate-800/70">
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="mt-1 break-words text-sm font-semibold text-slate-700 dark:text-slate-100">{value}</p>
      </div>
    </div>
  )
}
...
export function FileDetailsDrawer({ open, file, onClose }: { open: boolean; file: FileItem | null; onClose: () => void }) {
  return (
    <>
      <button className={open ? 'fixed inset-0 z-40 bg-slate-950/30 dark:bg-black/55' : 'pointer-events-none fixed inset-0 z-40 bg-slate-950/0'} aria-label="Close file details" onClick={onClose} />
      <aside className={open ? 'fixed right-0 top-0 z-50 h-full w-full max-w-md translate-x-0 border-l border-slate-200 bg-white shadow-2xl transition-transform duration-300 dark:border-slate-700 dark:bg-slate-900' : 'fixed right-0 top-0 z-50 h-full w-full max-w-md translate-x-full border-l border-slate-200 bg-white shadow-2xl transition-transform duration-300 dark:border-slate-700 dark:bg-slate-900'}>
        <div className="flex items-center justify-between border-b border-slate-200 p-5 dark:border-slate-700">
          <div>
            <h2 className="text-xl font-extrabold text-slate-900 dark:text-slate-50">File Details</h2>
            <p className="mt-1 max-w-[18rem] truncate text-sm text-slate-500 dark:text-slate-400">{file?.name ?? 'No file selected'}</p>
          </div>
