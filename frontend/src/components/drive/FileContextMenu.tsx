...
export function FileContextMenu({ x, y, file, onClose, onView, onDownload, onRename, onMove, onDetails, onShare, onInvite, onDelete }: { x: number; y: number; file: FileItem | null; onClose: () => void; onView: () => void; onDownload: () => void; onRename: () => void; onMove: () => void; onDetails: () => void; onShare: () => void; onInvite: () => void; onDelete: () => void }) {
  ...
  const safeY = Math.max(12, Math.min(y, window.innerHeight - 420))

  return (
    <>
      <button className="fixed inset-0 z-40 cursor-default bg-slate-950/20 dark:bg-black/50 sm:bg-transparent sm:dark:bg-transparent" aria-label="Close file menu" onClick={onClose} />
      <div className="fixed inset-x-3 bottom-3 z-50 rounded-3xl border border-slate-200 bg-white p-2 shadow-2xl shadow-slate-900/15 dark:border-slate-700 dark:bg-slate-900 dark:shadow-black/40 sm:inset-x-auto sm:bottom-auto sm:w-52 sm:rounded-2xl" style={{ left: window.innerWidth >= 640 ? safeX : undefined, top: window.innerWidth >= 640 ? safeY : undefined }}>
        <p className="truncate px-3 py-2 text-xs font-bold text-slate-500 dark:text-slate-400">{file.name}</p>
